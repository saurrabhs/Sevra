    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1 text-xs text-white/70 ring-1 ring-white/10">
              ...
              Pre-deployment static scan 
            </div>
            <h1 className="text-2xl font-semibold tracking-tight text-white md:text-3xl">
              AI Internal Security Analyst
            </h1>
            <p className="max-w-2xl text-sm text-white/70">
              Scan a demo project for common security mistakes, score risk, and get an AI-assisted remediation explanation.
            </p>
          </div>
          ...
          <section className="lg:col-span-1">
            <div className="rounded-2xl bg-white/5 p-5 ring-1 ring-white/10">
              ...
              <div className="mt-4 text-xs text-white/50">
                {data ? (
                  <div>
                    Scanned <span className="text-white/70">{data.fileCount}</span> files in <span className="text-white/70">{data.scannedPath}</span>
                  </div>
                ) : (
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-xs text-white/60">
                    Gemini analyzes one detected issue and suggests fixes.
                  </div>
                </div>
                <button
                    ...
                    data.findings.map((f: Finding) => (
                      ...
                      >
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          ...
                          <div className={`inline-flex w-fit items-center rounded-full px-2.5 py-1 text-xs ${severityClasses(f.severity)}`}>
                            {f.severity}
                          </div>
                        </div>
        ...
        <footer className="mt-10 text-xs text-white/40">
          MVP demo only. No uploads. No history. Local scan of backend demo folder.
        </footer>
      </div>
    </div>
  );
